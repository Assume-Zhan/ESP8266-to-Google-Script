...
const sheet_id = '1J-I7FNb21o1I4dK67IdFz3vBgxhOO_NxutIP651Xz78';
...
function doGet(e) {
  ...
  var return_value = {
    'value1': sheet.getRange('F1').getValue()
  };
  var value = sheet.getRange('F1').getValue()

  //return ContentService.createTextOutput(JSON.stringify(return_value)).setMimeType(ContentService.MimeType.JSON);
...
function doPost(e) {
  ...
  if (parsedData !== undefined){
    ...
    var date_now = Utilities.formatDate(new Date(), "EST", "yyyy/MM/dd"); // gets the current date
    var time_now = Utilities.formatDate(new Date(), "EST", "hh:mm:ss a"); // gets the current time
...
function doPostTest() {
  let blob = Utilities.newBlob(JSON.stringify({
    sheet_name: 'test',
    command: 'insert_row',
    values:'0,123,45678'
  }));
  blob.contents = blob.getDataAsString();
  const e = {
    postData: blob
  };
  const ret = doPost(e);
